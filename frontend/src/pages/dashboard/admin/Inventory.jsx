...
const Inventory = ({ items, loading, onEdit, onDelete }) => {
  return (
    <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-premium overflow-hidden min-h-[500px] flex flex-col animate-fade-in">
      {loading ? (
        <div className="flex-1 flex flex-col items-center justify-center p-20 text-slate-300">
          ...
          <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">Syncing inventory...</span>
        </div>
      ) : items.length === 0 ? (
        <div className="flex-1 flex flex-col items-center justify-center p-20 text-center animate-fade-in">
          ...
          <h4 className="text-lg font-black text-slate-800 tracking-tight">No Products Found</h4>
          <p className="text-sm text-slate-400 font-medium max-w-xs mx-auto mt-2">Try adding a new product to your inventory.</p>
        </div>
      ) : (
        <div className="overflow-x-auto custom-scrollbar">
          <ProductTable items={items} onEdit={onEdit} onDelete={onDelete} />
        </div>
      )}
    </div>
  );
